...
//                    [self presentViewController:alertVC animated:YES completion:nil];
                    ...
                    [self showMyAlert:@"Error" andError:error];
...
//                [self presentViewController:alertVC animated:YES completion:nil];
                ...
                [self showMyAlert:@"Warning" andError:nil];
            }
        });
    }];
}
...
// Alert View (refactored)
- (void) showMyAlert:(NSString *)myMessage andError:(NSError *)error {
    
    if ([myMessage  isEqual: @"Error"]) {
        // Error alert
        ...
        // Warning alert
        UIAlertController *alertVC2 = [UIAlertController alertControllerWithTitle:NSLocalizedString(@"Camera Permission Denied", @"camera permission denied title")
                                                                          message:NSLocalizedString(@"This app doesn't have permission to use the camera; please update your privacy settings.", @"camera permission denied recovery suggestion")
                                                                   preferredStyle:UIAlertControllerStyleAlert];
        
        [alertVC2 addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"OK", @"OK button")
                                                     style:UIAlertActionStyleCancel
                                                   handler:^(UIAlertAction *action) {
                                                       [self.delegate cameraViewController:self didCompleteWithImage:nil];
                                                   }]];
